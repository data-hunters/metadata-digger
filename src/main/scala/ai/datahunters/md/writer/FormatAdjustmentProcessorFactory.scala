...
import ai.datahunters.md.processor.{FlattenArrays, FlattenMetadataTags, Processor}
...
object FormatAdjustmentProcessorFactory {
  ...
  def create(config: MetadataEnrichmentConfig, outputFormat: String): Option[Processor] = outputFormat match {
    case FileOutputWriter.CsvFormat => Some(new FlattenArrays(config.labelsOutputDelimiter, Seq(MultiLabelPredictionSchemaConfig.LabelsCol)))
    case FileOutputWriter.JsonFormat => None
