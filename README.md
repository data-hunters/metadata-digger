...
### Setting up allowed file types filter
If you want to filter images, displaying only the ones with specific file type you should use property called `filter.allowedFileTypes`. You should input file types that you want to display seperating them with comma (`,`). Commenting out or not providing this property will result in not filtering images with this filter,  but leaving this property with empty value will not produce any data for images, since it means that no file types are allowed.
